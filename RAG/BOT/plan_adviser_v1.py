...
with open(file_path, 'rb') as file:
    ...
    if response.status_code == 200:
        ...
        bot_plan = make_plan(db_path = "./RAG/Database/Output", task = task,max_docs = 3,temp=0.7)

        bot_advice = bot.ask(question="Provide pointers, tips and overall advice for improving the group project plan proposal. In the context you have the extracted text from the plan proposal as well as in the end you have a summarized steps only plan. Look at both but place more focus on the actual plan proposal text and give useful feedback to the students on what they need to improve.", context=plan_as_text+"    Plan Super Short Summary:  "+bot_plan)
